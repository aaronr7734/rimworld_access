...
{
    /// <summary>
    /// Harmony patch that intercepts keyboard input when the outfit policy management interface is active.
    /// </summary>
    [HarmonyPatch(typeof(UIRoot))]
    [HarmonyPatch("UIRootOnGUI")]
    public static class OutfitPolicyPatch
    {
        /// <summary>
        /// Prefix patch that intercepts keyboard events when outfit policy manager is active.
        /// </summary>
        [HarmonyPrefix]
        [HarmonyPriority(Priority.First)]
        public static void Prefix()
        {
            // Only process if outfit policy manager is active
            ...
            else
            {
                ...
                var mode = WindowlessOutfitPolicyState.CurrentMode;

                if (mode == WindowlessOutfitPolicyState.NavigationMode.PolicyList)
                {
                    // Policy list mode
                    if (key == KeyCode.UpArrow)
                    {
                        WindowlessOutfitPolicyState.SelectPreviousPolicy();
                        handled = true;
                    }
                    else if (key == KeyCode.DownArrow)
                    {
                        WindowlessOutfitPolicyState.SelectNextPolicy();
                        handled = true;
                    }
                    else if (key == KeyCode.Tab && !Event.current.shift)
                    {
                        WindowlessOutfitPolicyState.EnterActionsMode();
                        handled = true;
                    }
                    else if (key == KeyCode.Escape)
                    {
                ...
                else if (mode == WindowlessOutfitPolicyState.NavigationMode.PolicyActions)
                {
                    // Actions menu mode
                    if (key == KeyCode.UpArrow)
                    {
                        WindowlessOutfitPolicyState.SelectPreviousAction();
                        handled = true;
                    }
                    else if (key == KeyCode.DownArrow)
                    {
                        WindowlessOutfitPolicyState.SelectNextAction();
                        handled = true;
                    }
                    else if (key == KeyCode.Tab && !Event.current.shift)
                    {
                    ...
                    {
                        WindowlessOutfitPolicyState.ExecuteAction();
                        handled = true;
                    }
                    else if (key == KeyCode.Escape)
                    {
        ...
        /// <summary>
        /// Postfix patch that draws visual feedback for the outfit policy manager.
        /// </summary>
        [HarmonyPostfix]
        public static void Postfix()
        {
            // Only draw if outfit policy manager is active
            if (!WindowlessOutfitPolicyState.IsActive)
                return;
        ...
        /// <summary>
        /// Draws a visual overlay indicating the outfit policy manager is active.
        /// </summary>
        private static void DrawMenuOverlay()
        {
            ...
            string title = "Outfit Policy Manager";
            string instructions1 = "Up/Down: Navigate | Tab: Switch Mode | Enter: Execute Action";
            string instructions2 = "N: New Policy | F2: Rename | D: Duplicate | Del: Delete | Esc: Close";
