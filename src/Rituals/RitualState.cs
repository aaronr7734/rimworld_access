    ...
    public static class RitualState
    {
        ...
        public static void Open(Dialog_BeginRitual dialog)
        {
            ...
            try
            {
                ...
                sb.Append("RimWorldAccess.Rituals.Open.Begin".Translate(ritualName));
                ...
                {
                    sb.Append(" ");
                    sb.Append("RimWorldAccess.Rituals.Open.Warning".Translate(warnings));
                }

                sb.Append(" ");
                sb.Append(roleCount == 1
                    ? "RimWorldAccess.Rituals.Open.RoleCountOne".Translate(roleCount).ToString()
                    : "RimWorldAccess.Rituals.Open.RoleCountMany".Translate(roleCount).ToString());
                sb.Append(" ");
        ...
        private static bool HandleTypeahead(KeyCode key, bool shift, bool ctrl, bool alt, List<string> labels, int currentIndex, Action<int> setIndex, Action announceAction)
        {
            ...
            // Letter/number keys for search — character dispatch is handled by the
            // layout-aware TypeaheadDispatcher (see TypeaheadConsumerRegistry).
            // Consume the KeyCode here so the game's defaults don't fire.
        ...
        /// <summary>
        /// Routes a layout-aware typeahead character to the active mode's list.
        /// Called from TypeaheadDispatcher (registered in TypeaheadConsumerRegistry).
        /// </summary>
        public static void HandleTypeahead(char c)
        {
            ...
            if (currentMode == NavigationMode.RoleList)
            {
                if (roleItems == null) return;
                labels = roleItems.Select(r => r.Label).ToList();
                currentIndex = roleIndex;
                setIndex = i => roleIndex = i;
                announceAction = AnnounceCurrentRole;
            }
            else if (currentMode == NavigationMode.PawnSelection)
            {
                if (pawnItems == null) return;
                labels = pawnItems.Select(p => p.Pawn.LabelShort).ToList();
            ...
            {
                return; // QualityStats has no typeahead
            }
        ...
        private static void BuildQualityStatsList()
        {
            ...
            try
            {
                ...
                if (populateMethod != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Label = "RimWorldAccess.Rituals.Quality.NoQualityFactorsLabel".Translate(),
                            Change = "",
                            IsPresent = true,
                            IsPositive = true,
                            IsUncertain = false,
                            Tooltip = "RimWorldAccess.Rituals.Quality.NoQualityFactorsTooltip".Translate(),
                            Explanation = null
                        });
                    }
                    ...
                    if (currentTarget.IsValid)
                    {
                        ...
                        if (!string.IsNullOrEmpty(locationText))
                        {
                            ...
                            {
                                Label = "RimWorldAccess.Rituals.Quality.LocationLabel".Translate(),
                                Change = locationText,
            ...
            catch (Exception ex)
            {
                ...
                qualityItems.Add(new RitualQualityStatItem
                {
                    Label = "RimWorldAccess.Rituals.Quality.Unavailable".Translate(),
                    Change = "",
                    IsPresent = false,
                    IsPositive = true,
                    IsUncertain = false,
        ...
        private static void AddIdeologyDevelopmentPoints()
        {
            ...
            try
            {
                ...
                qualityItems.Add(new RitualQualityStatItem
                {
                    Label = "RimWorldAccess.Rituals.Quality.DevelopmentPointsLabel".Translate(),
                    Change = "RimWorldAccess.Rituals.Quality.FluidIdeologyTag".Translate(),
                    IsPresent = true,
                    IsPositive = true,
                    IsUncertain = false,
                    Tooltip = "RimWorldAccess.Rituals.Quality.DevelopmentPointsTooltip".Translate(),
                    Explanation = null
                });
        ...
        private static string GetExpectedQualitySummary(Dialog_BeginRitual dialog)
        {
            try
            {
                ...
                if (populateMethod != null)
                {
                    ...
                    {
                        return "RimWorldAccess.Rituals.Open.ExpectedQualitySingle".Translate(qualityRange.min.ToStringPercent("F0")).ToString();
                    }
                    return "RimWorldAccess.Rituals.Open.ExpectedQualityRange".Translate(qualityRange.min.ToStringPercent("F0"), qualityRange.max.ToStringPercent("F0")).ToString();
                }
            }
            catch
            {
        ...
        private static string GetBlockingIssues(Dialog_BeginRitual dialog)
        {
            ...
            try
            {
                ...
                if (currentAssignments != null)
                {
                    ...
                    if (hasDrafted)
                    {
                        allIssues.Add("RimWorldAccess.Rituals.Warning.DraftedParticipants".Translate());
                    }
                }
            }
            catch
        ...
        private static string GetLocationDescription()
        {
                ...
                try
                {
                    ...
                    if (map != null && cell.IsValid && cell.InBounds(map))
                    {
                        ...
                        if (room != null && room.ProperRoom && !room.PsychologicallyOutdoors)
                        {
                            ...
                            {
                                parts.Add("RimWorldAccess.Rituals.Location.InRoom".Translate(roomLabel).ToString());
                            }
                        }
                        else if (room != null && room.PsychologicallyOutdoors)
                        {
                            parts.Add("RimWorldAccess.Rituals.Location.Outdoors".Translate().ToString());
                        }
                    }
                }
                catch
            ...
            {
                return "RimWorldAccess.Rituals.Location.Cell".Translate(currentTarget.Cell.x, currentTarget.Cell.z);
            }
